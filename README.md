...
2. Активируем виртуальное окружение poetry
```poetry shell```

3. Создать файл .env и заполнить его переменными окружения
```cp .env.template .env```

4. Запуск контейнера с PostgreSQL ```docker-compose up -d```

5. Настройка базы данных.

 - Переходим в папку с приложением: ```cd src```

 - генерируем миграции и формируем таблицы в БД

```python -m migration_tool revision --autogenerate && python -m migration_tool upgrade head```

6. Запуск локального сервера веб приложения: ```python server.py```
